...
class Provider15Error extends Error {
  constructor(message, code, originalError = null) {
    super(message);
    this.name = 'Provider15Error';
    this.code = code;
...
class Provider15 extends ProviderInterface {
    ...
    async createSession() {
        ...
        try {
            ...
            if (response.status !== 200 || !response.data.startsWith("0")) {
                throw new Provider15Error("Invalid response format", "INVALID_RESPONSE");
            }
            ...
            if (postResponse.status !== 200 || postResponse.data !== "OK") {
                throw new Provider15Error("Invalid post response", "INVALID_POST_RESPONSE");
            }

            return { sid, t };
        } catch (error) {
            throw new Provider15Error("Failed to create session", "SESSION_CREATION_ERROR", error);
        }
    }
    ...
    async *generateCompletionStream(messages, temperature, max_tokens, functions, function_call) {
        ...
        while (retries < this.maxRetries) {
            try {
                ...
                Logger.debug('Provider15: WebSocket connected');
                ...
                while (true) {
                    ...
                    try {
                        ...
                        if (!data || !data.output) {
                            Logger.error('Provider15: No output in received data:', data);
                            continue;
                        }
                        ...
                        Logger.error(`Provider15: Message parsing error: ${message}`, error);
                        continue;
                    }
                }
                ...
                Logger.error(`Provider15: Error in generateCompletionStream (attempt ${retries + 1}):`, error);
                retries++;
                if (retries < this.maxRetries) {
                    await sleep(this.retryDelay * Math.pow(2, retries));
                } else {
                    throw new Provider15Error("Failed to generate completion stream after multiple attempts", "STREAM_GENERATION_ERROR", error);
                }
            }
        }
    }
    ...
    async sendAndWait(ws, sendMessage, expectedResponse) {
        ...
        if (response.toString() !== expectedResponse) {
            throw new Provider15Error(`Unexpected response: ${response}, expected: ${expectedResponse}`, "UNEXPECTED_RESPONSE");
        }
    }
    ...
    async waitForMessage(ws, condition) {
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                ws.removeListener('message', handler);
                reject(new Provider15Error('Timeout waiting for message', "TIMEOUT_ERROR"));
            }, 10000);
        });
    }
        ...
        while (retries < this.maxRetries) {
            try {
                ...
                if (completionChunks.length === 0) {
                    throw new Provider15Error('No content received from stream', "NO_CONTENT_ERROR");
                }
                ...
                Logger.error(`Provider15: Error in generateCompletion (attempt ${retries + 1}):`, error);
                retries++;
                if (retries < this.maxRetries) {
                    await sleep(this.retryDelay * Math.pow(2, retries));
                } else {
                    throw new Provider15Error("Failed to generate completion after multiple attempts", "COMPLETION_GENERATION_ERROR", error);
                }
            }
        }
    }
