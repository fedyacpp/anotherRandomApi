...
class Provider4 extends ProviderInterface {
    constructor() {
        ...
        this.modelInfo = {
            modelId: "gpt-4o",
            name: "gpt-4o",
            description: "OpenAI's latest iteration, showcasing improvements in various domains while maintaining efficiency",
    ...
    async refreshAuthCode() {
        try {
            ...
            throw new Provider4Error('Failed to refresh authentication', 'AUTH_REFRESH_ERROR', error);
        }
    }
    ...
    async makeRequest(endpoint, data, stream = false) {
        ...
        try {
            ...
            throw new Provider4Error(`Error making request to ${endpoint}`, 'REQUEST_ERROR', error);
        }
    }
            ...
            try {
                ...
                for await (const chunk of response.data) {
                    ...
                    if (chunkStr.includes('<html')) {
                        throw new Provider4Error('Invalid session', 'INVALID_SESSION');
                    }
                ...
                if (error instanceof Provider4Error && error.code === 'INVALID_SESSION') {
                    await this.refreshAuthCode();
