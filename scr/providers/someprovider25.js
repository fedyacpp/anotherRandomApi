...
class Provider25 extends ProviderInterface {
    constructor() {
        ...
        this.modelInfo = {
            modelId: "gpt-4o-mini",
            name: "gpt-4o-mini",
            description: "A newest and fastest model from OpenAI",
    ...
    async refreshAuthCode() {
        try {
            ...
            throw new Provider25Error('Failed to refresh authentication', 'AUTH_REFRESH_ERROR', error);
        }
    }
    ...
    async makeRequest(endpoint, data, stream = false) {
        ...
        try {
            ...
            throw new Provider25Error(`Error making request to ${endpoint}`, 'REQUEST_ERROR', error);
        }
    }
            ...
            try {
                ...
                for (const chunk of response.data) {
                    ...
                    if (chunkStr.includes('<html')) {
                        throw new Provider25Error('Invalid session', 'INVALID_SESSION');
                    }
                ...
                if (error instanceof Provider25Error && error.code === 'INVALID_SESSION') {
                    await this.refreshAuthCode();
