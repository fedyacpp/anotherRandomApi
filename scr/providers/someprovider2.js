...
class Provider2 extends ProviderInterface {
    constructor() {
        ...
        this.modelInfo = {
            ...
            description: "A powerful language model that balances performance and efficiency, often used for complex tasks",
    ...
    async refreshAuthCode() {
        try {
            ...
            throw new Provider2Error('Failed to refresh authentication', 'AUTH_REFRESH_ERROR', error);
        }
    }
    ...
    async makeRequest(endpoint, data, stream = false) {
        ...
        try {
            ...
            throw new Provider2Error(`Error making request to ${endpoint}`, 'REQUEST_ERROR', error);
        }
    }
            ...
            try {
                ...
                for await (const chunk of response.data) {
                    ...
                    if (chunkStr.includes('<html')) {
                        throw new Provider2Error('Invalid session', 'INVALID_SESSION');
                    }
                ...
                if (error instanceof Provider2Error && error.code === 'INVALID_SESSION') {
                    await this.refreshAuthCode();
