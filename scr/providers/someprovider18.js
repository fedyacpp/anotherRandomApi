...
class Provider18Error extends Error {
  constructor(message, code, originalError = null) {
    super(message);
    this.name = 'Provider18Error';
    this.code = code;
    this.originalError = originalError;
  }
}

class Provider18 extends ProviderInterface {
    constructor() {
        super();
        ...
        this.modelInfo = {
            ...
            description: "A more compact version of Perplexity's LLaMA 3 adaptation, balancing efficiency and performance",
    ...
    async createSession() {
        ...
        try {
            ...
            if (response.status !== 200 || !response.data.startsWith("0")) {
                throw new Provider18Error("Invalid response format", "INVALID_RESPONSE");
            }
            ...
            if (postResponse.status !== 200 || postResponse.data !== "OK") {
                throw new Provider18Error("Invalid post response", "INVALID_POST_RESPONSE");
            }

            return { sid, t };
        } catch (error) {
            throw new Provider18Error("Failed to create session", "SESSION_CREATION_ERROR", error);
        }
    }
    ...
    async *generateCompletionStream(messages, temperature, max_tokens, functions, function_call) {
        ...
        while (retries < this.maxRetries) {
            try {
                ...
                Logger.debug('Provider18: WebSocket connected');
                ...
                while (true) {
                    ...
                    try {
                        ...
                        if (!data || !data.output) {
                            Logger.error('Provider18: No output in received data:', data);
                            continue;
                        }
                        ...
                        Logger.error(`Provider18: Message parsing error: ${message}`, error);
                        continue;
                    }
                }
                ...
                Logger.error(`Provider18: Error in generateCompletionStream (attempt ${retries + 1}):`, error);
                retries++;
                if (retries < this.maxRetries) {
                    await sleep(this.retryDelay * Math.pow(2, retries));
                } else {
                    throw new Provider18Error("Failed to generate completion stream after multiple attempts", "STREAM_GENERATION_ERROR", error);
                }
            }
        }
    }
    ...
    async sendAndWait(ws, sendMessage, expectedResponse) {
        ...
        if (response.toString() !== expectedResponse) {
            throw new Provider18Error(`Unexpected response: ${response}, expected: ${expectedResponse}`, "UNEXPECTED_RESPONSE");
        }
    }
    ...
    async waitForMessage(ws, condition) {
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                ws.removeListener('message', handler);
                reject(new Provider18Error('Timeout waiting for message', "TIMEOUT_ERROR"));
            }, 10000);
        });
    }
        ...
        while (retries < this.maxRetries) {
            try {
                ...
                if (completionChunks.length === 0) {
                    throw new Provider18Error('No content received from stream', "NO_CONTENT_ERROR");
                }
                ...
                Logger.error(`Provider18: Error in generateCompletion (attempt ${retries + 1}):`, error);
                retries++;
                if (retries < this.maxRetries) {
                    await sleep(this.retryDelay * Math.pow(2, retries));
                } else {
                    throw new Provider18Error("Failed to generate completion after multiple attempts", "COMPLETION_GENERATION_ERROR", error);
                }
            }
        }
    }
