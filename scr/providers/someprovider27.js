...
class Provider27Error extends Error {
  constructor(message, code, originalError = null) {
    super(message);
    this.name = 'Provider27Error';
    this.code = code;
    this.originalError = originalError;
  }
}

class Provider27 extends ProviderInterface {
    constructor() {
        super();
        this.baseUrl = "https://api.deepinfra.com";
        this.modelName = "mistralai/Mixtral-8x22B-Instruct-v0.1"
        this.modelInfo = {
    ...
    async makeRequest(endpoint, data, stream = false) {
        ...
        try {
            ...
            throw new Provider27Error(`Error making request to ${endpoint}`, 'REQUEST_ERROR', error);
        }
    }
        ...
        for (let attempt = 0; attempt < this.maxAttempts; attempt++) {
            try {
                ...
                if (attempt === this.maxAttempts - 1) {
                    throw new Provider27Error('Failed to generate completion', 'COMPLETION_ERROR', error);
                }
            }
        }
    }
    ...
    async *generateCompletionStream(messages, temperature, max_tokens) {
        for (let attempt = 0; attempt < this.maxAttempts; attempt++) {
            try {
                ...
                if (attempt === this.maxAttempts - 1) {
                    throw new Provider27Error('Failed to generate completion stream', 'STREAM_ERROR', error);
                }
            }
        }
    }
