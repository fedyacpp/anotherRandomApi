...
class Provider21Error extends Error {
  constructor(message, code, originalError = null) {
    super(message);
    this.name = 'Provider21Error';
    this.code = code;
  ...
  async getInitialVqd() {
    ...
    while (retries < this.maxRetries) {
      try {
        ...
        if (!this.vqd) {
          throw new Provider21Error('Failed to extract vqd from status response', 'VQD_EXTRACTION_ERROR');
        }

        Logger.info(`Provider21: Successfully extracted initial vqd: ${this.vqd}`);
        return this.vqd;
      } catch (error) {
        Logger.error(`Provider21: Error in getInitialVqd (attempt ${retries + 1}):`, error);
        retries++;
        if (retries < this.maxRetries) {
          await sleep(this.retryDelay * Math.pow(2, retries));
        } else {
          throw new Provider21Error("Failed to get initial vqd after multiple attempts", "INITIAL_VQD_ERROR", error);
        }
      }
    }
  }
  ...
  async *generateCompletionStream(messages) {
    ...
    while (retries < this.maxRetries) {
      try {
          ...
          for (const line of lines) {
            if (line.startsWith('data: ')) {
              try {
                ...
                Logger.warn('Provider21: Error parsing stream data:', parseError);
              }
            }
          }
        }
        ...
        Logger.error(`Provider21: Error in generateCompletionStream (attempt ${retries + 1}):`, error);
        retries++;
        if (retries < this.maxRetries) {
          await sleep(this.retryDelay * Math.pow(2, retries));
        } else {
          throw new Provider21Error("Failed to generate completion stream after multiple attempts", "STREAM_GENERATION_ERROR", error);
        }
      }
    }
  }
    ...
    while (retries < this.maxRetries) {
      try {
        ...
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            try {
              ...
              Logger.warn('Provider21: Error parsing response data:', error);
            }
          }
        }
        ...
        if (fullMessage) {
          ...
          throw new Provider21Error('No content in provider response', 'EMPTY_RESPONSE');
        }
      } catch (error) {
        Logger.error(`Provider21: Error in generateCompletion (attempt ${retries + 1}):`, error);
        retries++;
        if (retries < this.maxRetries) {
          await sleep(this.retryDelay * Math.pow(2, retries));
        } else {
          throw new Provider21Error("Failed to generate completion after multiple attempts", "COMPLETION_GENERATION_ERROR", error);
        }
      }
    }
  }
