...
class Provider13Error extends Error {
  constructor(message, code, originalError = null) {
    super(message);
    this.name = 'Provider13Error';
    this.code = code;
    this.originalError = originalError;
  }
}

class Provider13 extends ProviderInterface {
  constructor() {
    super();
  ...
  async createSession() {
    ...
    try {
      ...
      const response = await axios.get(`${this.api_url}?EIO=4&transport=polling&t=${t}`, {
        headers: await this.getHeaders()
      });

      if (response.status !== 200 || !response.data.startsWith("0")) {
        throw new Provider13Error("Invalid response format", "INVALID_RESPONSE");
      }

      const sid = JSON.parse(response.data.slice(1)).sid;

      const postData = '40{"jwt":"anonymous-ask-user"}';
      const postResponse = await axios.post(`${this.api_url}?EIO=4&transport=polling&t=${t}&sid=${sid}`, postData, {
        headers: { ...await this.getHeaders(), 'Content-Type': 'text/plain;charset=UTF-8' },
      });

      if (postResponse.status !== 200 || postResponse.data !== "OK") {
        throw new Provider13Error("Invalid post response", "INVALID_POST_RESPONSE");
      }

      return { sid, t };
    } catch (error) {
      throw new Provider13Error("Failed to create session", "SESSION_CREATION_ERROR", error);
    }
  }
  ...
  async *generateCompletionStream(messages, temperature, max_tokens, functions, function_call) {
    ...
    while (retries < this.maxRetries) {
      try {
        ...
        Logger.debug('Provider13: WebSocket connected');
        ...
        while (true) {
          ...
          try {
            ...
            if (!data || !data.output) {
              Logger.error('Provider13: No output in received data:', data);
              continue;
            }
            ...
            Logger.error(`Provider13: Message parsing error: ${message}`, error);
            continue;
          }
        }
        ...
        Logger.error(`Provider13: Error in generateCompletionStream (attempt ${retries + 1}):`, error);
        retries++;
        if (retries < this.maxRetries) {
          await sleep(this.retryDelay * Math.pow(2, retries));
        } else {
          throw new Provider13Error("Failed to generate completion stream after multiple attempts", "STREAM_GENERATION_ERROR", error);
        }
      }
    }
  }
  ...
  async sendAndWait(ws, sendMessage, expectedResponse) {
    ...
    if (response.toString() !== expectedResponse) {
      throw new Provider13Error(`Unexpected response: ${response}, expected: ${expectedResponse}`, "UNEXPECTED_RESPONSE");
    }
  }
  ...
  async waitForMessage(ws, condition) {
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        ws.removeListener('message', handler);
        reject(new Provider13Error('Timeout waiting for message', "TIMEOUT_ERROR"));
      }, 10000);
    });
  }
  ...
  async generateCompletion(messages, temperature, max_tokens, functions, function_call) {
    ...
    while (retries < this.maxRetries) {
      try {
        await this.waitForRateLimit();
        const session = await this.createSession();
        const response = await axios.post(`${this.api_url}?EIO=4&transport=polling&t=${session.t}&sid=${session.sid}`, {
          model: this.modelInfo.modelId,
        ...
        if (response.status === 200) {
          ...
          throw new Provider13Error('Failed to generate completion', 'INVALID_RESPONSE');
        }
      } catch (error) {
        Logger.error(`Provider13: Error in generateCompletion (attempt ${retries + 1}):`, error);
        retries++;
        if (retries < this.maxRetries) {
          await sleep(this.retryDelay * Math.pow(2, retries));
        } else {
          throw new Provider13Error('Failed to generate completion after multiple attempts', 'COMPLETION_ERROR', error);
        }
      }
    }
  }
