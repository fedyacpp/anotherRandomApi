...
class Provider28Error extends Error {
  constructor(message, code, originalError = null) {
    super(message);
    this.name = 'Provider28Error';
    this.code = code;
    this.originalError = originalError;
  }
}

class Provider28 extends ProviderInterface {
    constructor() {
        super();
        this.baseUrl = "https://api.deepinfra.com";
        this.modelName = "meta-llama/Meta-Llama-3.1-405B-Instruct"
        this.modelInfo = {
    ...
    async makeRequest(endpoint, data, stream = false) {
        ...
        try {
            ...
            throw new Provider28Error(`Error making request to ${endpoint}`, 'REQUEST_ERROR', error);
        }
    }
        ...
        for (let attempt = 0; attempt < this.maxAttempts; attempt++) {
            try {
                ...
                if (attempt === this.maxAttempts - 1) {
                    throw new Provider28Error('Failed to generate completion', 'COMPLETION_ERROR', error);
                }
            }
        }
    }
    ...
    async *generateCompletionStream(messages, temperature, max_tokens) {
        for (let attempt = 0; attempt < this.maxAttempts; attempt++) {
            try {
                ...
                if (attempt === this.maxAttempts - 1) {
                    throw new Provider28Error('Failed to generate completion stream', 'STREAM_ERROR', error);
                }
            }
        }
    }
