...
class Provider11 extends ProviderInterface {
    constructor() {
        ...
        this.modelInfo = {
            ...
            description: "An AI model that combines extensive knowledge with nuanced understanding, excelling in complex reasoning tasks",
    ...
    async refreshAuthCode() {
        try {
            ...
            throw new Provider11Error('Failed to refresh authentication', 'AUTH_REFRESH_ERROR', error);
        }
    }
    ...
    async makeRequest(endpoint, data, stream = false) {
        ...
        try {
            ...
            throw new Provider11Error(`Error making request to ${endpoint}`, 'REQUEST_ERROR', error);
        }
    }
            ...
            try {
                ...
                for await (const chunk of response.data) {
                    ...
                    if (chunkStr.includes('<html')) {
                        throw new Provider11Error('Invalid session', 'INVALID_SESSION');
                    }
                ...
                if (error instanceof Provider11Error && error.code === 'INVALID_SESSION') {
                    await this.refreshAuthCode();
