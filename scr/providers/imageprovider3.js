...
class ImageProvider3Error extends Error {
  constructor(message, code, originalError = null) {
    super(message);
    this.name = 'ImageProvider3Error';
    this.code = code;
...
class ImageProvider3 extends ImageProviderInterface {
  ...
  async makeRequest(endpoint, params) {
    ...
    try {
      ...
      throw new ImageProvider3Error(`Error making request to ${endpoint}`, 'REQUEST_ERROR', error);
    }
  }
      ...
      for (let attempt = 0; attempt < this.maxAttempts; attempt++) {
        try {
          ...
          if (attempt === this.maxAttempts - 1) {
            throw new ImageProvider3Error('Failed to generate image', 'IMAGE_GENERATION_ERROR', error);
          }
        }
      }
    }
